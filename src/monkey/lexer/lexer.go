...
// peek ahead 1 char, but don't move forward
...
func (l *Lexer) NextToken() token.Token {
	...
	case '=':
		if l.peekChar() == '=' {
			...
			// Thus we don’t lose current character and can safely advance lexer
			// so it leaves NextToken() with l.position and l.readPosition in
			// the correct state.
			ch := l.ch
			l.readChar()
